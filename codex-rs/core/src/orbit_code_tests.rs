...
use orbit_code_protocol::protocol::NonSteerableTurnKind;
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let codex_home = session.orbit_code_home().await;
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.permissions.approval_policy =
            orbit_code_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        orbit_code_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
...
async fn build_test_config(codex_home: &Path) -> Config {
    ConfigBuilder::default()
        .orbit_code_home(codex_home.to_path_buf())
        .build()
        .await
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        orbit_code_exec_server::Environment::create(/*exec_server_url*/ None)
            .await
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let task_context = orbit_code_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
