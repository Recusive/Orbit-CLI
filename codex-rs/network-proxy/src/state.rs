...
pub fn build_config_state(
    ...
    let denied_domains = config.network.denied_domains.clone();
...
pub fn validate_policy_against_constraints(
    ...
    let config_allowed_domains = config.network.allowed_domains.clone();
    let config_denied_domains = config.network.denied_domains.clone();
    let denied_domain_overrides: HashSet<String> = config_denied_domains
        .iter()
        .map(|entry| entry.to_ascii_lowercase())
        .collect();
    let config_allow_unix_sockets = config.network.allow_unix_sockets.clone();
